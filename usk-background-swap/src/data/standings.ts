...
function teamIdFor(match: Match, side: "home" | "away") {
  if (side === "home") return match.homeTeamId || (match.home ? "usk" : match.opponentTeamId);
  return match.opponentTeamId || (match.home ? match.opponentTeamId : "usk");
}

function calculateFromMatches(teams: Team[], matches: Match[]): StandingTeam[] {
  ...
  matches
    ...
    .forEach((match) => {
      const homeId = teamIdFor(match, "home");
      const awayId = teamIdFor(match, "away");
      const home = homeId && rows.get(homeId);
      const away = awayId && rows.get(awayId);
      if (!home || !away || home.id === away.id) return;
      const homeGoals = match.home ? match.scoreFor! : match.scoreAgainst!;
      const awayGoals = match.home ? match.scoreAgainst! : match.scoreFor!;
...
export function getStandings(): StandingTeam[] {
  ...
  if (
    ...
    return calculateFromMatches(getTeams(), matches);
  }
  try {
