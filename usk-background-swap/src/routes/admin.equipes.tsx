...
function AdminTeamsPage() {
  ...
  const save = async () => {
    ...
    try {
      ...
      let logo = form.logo;
      if (pendingLogoFile) {
        const path = `teams/${id}/logo-${Date.now()}-${pendingLogoFile.name.replace(/[^a-zA-Z0-9.-]/g, "-")}`;
        logo = (await uploadAsset({ data: { path, file: pendingLogoFile } })) || path;
      }
      ...
      const next: Team = {
        ...
        ...form,
        logo,
        virtualKit: logoChanged
          ? { status: "pending", generatedAt: null, logo: logo || null }
          : editing?.virtualKit,
      };
      await saveTeams(
        editing ? teams.map((team) => (team.id === editing.id ? next : team)) : [...teams, next],
      );
      close();
    } catch {
      window.alert("L’équipe n’a pas pu être enregistrée. Vérifiez le stockage puis réessayez.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const reset = () => {
    if (window.confirm("Restaurer les 14 équipes officielles ?")) saveTeams(officialTeams);
  };
  const generateKit = (teamId: string) => {
  ...
  const uploadLogo = async (teamId: string, file: File) => {
    ...
    try {
      const logo = await uploadAsset({ data: { path, file } });
      await updateTeamLogo(teams, teamId, logo || path);
    } catch (error) {
      if (import.meta.env.DEV) console.error("[v0] Team logo flow failed", error);
              ...
              <label className="admin-logo-upload">
                ...
                <input
                  ...
                  onChange={(event) => {
                    ...
                    setForm((current) => ({ ...current, virtualKit: undefined }));
                  }}
                />
