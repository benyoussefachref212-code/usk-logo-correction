...
const defaultSeason = "2026 / 2027";
const selectedSeasonStorageKey = "usk-admin-selected-season";

function isSeasonLabel(value: string | undefined) {
  return Boolean(value && /^\d{4}\s*[\/\-–]\s*\d{4}$/.test(value.trim()));
}

function getMatchSeason(match: Match) {
  const season = match.competition.split(" · ").find(isSeasonLabel);
  return season?.trim() || "2025 / 2026";
}

// ✅ Extrait le numéro de journée d'un match (ex: "Journée 3" -> 3)
function getMatchRound(match: Match): number | null {
  const parts = match.competition.split(" · ");
  const roundPart = parts.find((part) => /^journ[ée]e\s*\d+/i.test(part.trim()));
  if (!roundPart) return null;
  const m = roundPart.match(/(\d+)/);
  return m ? Number(m[1]) : null;
}

function getMatchRoundLabel(match: Match) {
  const roundPart = match.competition
    .split(" · ")
    .find((part) => /^journ[ée]e\s*\d+/i.test(part.trim()));
  return roundPart?.trim() || "Journée non définie";
}

// ✅ Normalise une saisie libre (ex: "2025/2026" -> "2025 / 2026")
...
function AdminMatchsPage() {
  ...
  const [selectedSeason, setSelectedSeason] = useState(() => {
    if (typeof window === "undefined") return defaultSeason;
    return window.localStorage.getItem(selectedSeasonStorageKey) || defaultSeason;
  });
  // ✅ Journée sélectionnée (input libre, valeur par défaut = 1)
  const [selectedRound, setSelectedRound] = useState("1");
  // ✅ Case à cocher pour afficher TOUTES les journées
  const [allRounds, setAllRounds] = useState(false);

  useEffect(() => {
    window.localStorage.setItem(selectedSeasonStorageKey, normalizeSeason(selectedSeason));
  }, [selectedSeason]);
  ...
  const remove = async (id: string) => {
    ...
    try {
      ...
      setSaveMessage(
        message.includes("usk_matches")
          ? "La table usk_matches n’est pas encore créée dans Supabase. Appliquez la migration 0002_verified_schema.sql."
          : `Impossible de supprimer le match : ${message}`,
      );
    } finally {
  ...
  const filtered = useMemo(
    () =>
      matches.filter((match) => {
        const matchesSeason = getMatchSeason(match) === selectedSeason;

        // ✅ Filtrage par journée
        const roundNumber = Number(selectedRound);
        const matchRound = getMatchRound(match);
        const matchesRound =
          allRounds || (Number.isFinite(roundNumber) && matchRound === roundNumber);

        const matchesTab = activeTab === "all" || match.status === activeTab;
        const needle = normalizeSearchText(query.trim());
        const opponentTeam = match.opponentTeamId
          ? getTeamById(teams, match.opponentTeamId)
          : undefined;
        const homeTeam = match.homeTeamId ? getTeamById(teams, match.homeTeamId) : undefined;
    ...
    [matches, query, activeTab, teams, selectedSeason, selectedRound, allRounds],
  );

  const seasonMatches = matches.filter((match) => getMatchSeason(match) === selectedSeason);
  const counts = {
    all: seasonMatches.length,
    upcoming: seasonMatches.filter((match) => match.status === "upcoming").length,
    live: seasonMatches.filter((match) => match.status === "live").length,
    finished: seasonMatches.filter((match) => match.status === "finished").length,
    postponed: seasonMatches.filter((match) => match.status === "postponed").length,
    cancelled: seasonMatches.filter((match) => match.status === "cancelled").length,
  };
  ...
  const open = (match?: Match) => {
    ...
    setForm(
      match
        ...
        : {
            ...empty,
            competition: `Championnat · ${normalizeSeason(selectedSeason)} · Journée 1 · Phase aller`,
  ...
  const save = async () => {
    ...
    if (form.status !== "finished") {
      const scheduledAt = new Date(`${form.date}T${form.time || "00:00"}`);
      if (Number.isNaN(scheduledAt.getTime()) || scheduledAt <= new Date()) {
        setSaveMessage("Impossible : choisissez une date et une heure futures pour ce match.");
        return;
      }
    }
    ...
    const clean = normalizeMatch({
      ...
      ...(form.status === "finished"
        ? { scoreFor: form.scoreFor ?? 0, scoreAgainst: form.scoreAgainst ?? 0 }
        : {}),
      stadium: homeStadium,
  ...
  return (
    <main className="reference-matches-page">
      ...
      <div className="reference-matches-toolbar">
        ...
        {/* ✅ Saison librement modifiable */}
        <label className="reference-match-season">
          ...
          <input
            ...
            onBlur={(event) => setSelectedSeason(normalizeSeason(event.target.value))}
            placeholder={defaultSeason}
            aria-label="Saison"
          />
        </label>

        {/* ✅ Journée : input libre + case "Toutes les journées" */}
        <label className="reference-match-round">
          <List size={17} />
          <span className="sr-only">Journée</span>
          <span className="reference-match-round-label">Journée</span>
          <input
            type="number"
            min={1}
            value={selectedRound}
            onChange={(event) => {
              ...
              if (allRounds) setAllRounds(false);
            }}
            placeholder="1"
            aria-label="Numéro de journée"
            disabled={allRounds}
          />
        </label>

        <label className="reference-match-all-rounds">
          <input
            type="checkbox"
            checked={allRounds}
            onChange={(event) => setAllRounds(event.target.checked)}
          />
      ...
      <section className="reference-match-list">
        <div className="reference-list-title">
          <h2>
            Rencontres{" "}
            {!allRounds && Number.isFinite(Number(selectedRound))
          ...
          {filtered.length ? (
            filtered.map((match) => (
              <MatchRow
                ...
                onToggleStatus={() =>
                  setStatusMenuId(statusMenuId === match.id ? null : match.id)
                }
              />
            ))
          ) : (
            <div className="reference-empty-state">
              Aucun match ne correspond à cette journée.
            </div>
          )}
        </div>
      ...
      {pendingDelete && (
        <div
          className="match-delete-backdrop"
          role="presentation"
          onMouseDown={() => !deleteSaving && setPendingDelete(null)}
        >
          <section
          ...
          >
            <div className="match-delete-icon" aria-hidden="true">
              <Trash2 size={21} />
            </div>
            <h2 id="match-delete-title">Supprimer ce match ?</h2>
            <p id="match-delete-description">
              Cette action supprimera définitivement le match contre{" "}
              <strong>{pendingDelete.opponent}</strong>.
            </p>
            <div className="match-delete-actions">
              <button
                type="button"
                className="match-delete-cancel"
              ...
              <button
                type="button"
                className="match-delete-confirm"
                disabled={deleteSaving}
...
function MatchRow({
  ...
  <div>
    <strong>{match.competition.split(" · ")[0]}</strong>
            <small>
              {getMatchRoundLabel(match)} · {getMatchSeason(match)}
            </small>
  </div>
</div>
      ...
      <div
        className={`reference-match-time${
          match.status === "finished" ? " reference-match-time-finished" : ""
        }`}
      >
      ...
      >
        ...
        <div className="match-details-grid">
          ...
          <div className="match-details-info">
            ...
            <span>
              <small>SAISON</small>
              <strong>{match.competition.split(" · ")[1] || defaultSeason}</strong>
              <em>{getMatchRoundLabel(match)}</em>
            </span>
          </div>
        </div>
...
function Editor({
  ...
  const seasonIndex = parts.findIndex(isSeasonLabel);
  const season: string = (seasonIndex >= 0 ? parts[seasonIndex] : undefined) ?? defaultSeason;
  const round =
    seasonIndex >= 0 ? parts[seasonIndex + 1] || "" : parts[1] || (isCup ? "" : "Journée 1");
  const phase =
    seasonIndex >= 0 ? parts[seasonIndex + 2] || "Phase aller" : parts[2] || "Phase aller";
  const [seasonDraft, setSeasonDraft] = useState(season);
  const [roundDraft, setRoundDraft] = useState(round.match(/\d+/)?.[0] || "1");
    ...
    update({
      competition:
        next === "Coupe de Tunisie"
          ? composeCompetition(next, seasonDraft, "", "")
          : composeCompetition(next, seasonDraft, "Journée 1", "Phase aller"),
    });
  };
    ...
    <div className="admin-editor-backdrop">
      ...
      >
        ...
        <EditorSection number="2" title="Date et heure">
          <div className="match-editor-fields two-columns">
            <IconField icon={CalendarDays} label="Date du match">
              <input
                type="date"
                min={form.status === "finished" ? undefined : getTodayDateInputValue()}
                value={form.date}
          ...
          <div className={`match-editor-fields ${isCup ? "three-columns" : "four-columns"}`}>
            ...
            <label>
              ...
              <input
                ...
                placeholder={defaultSeason}
                onChange={(event) => {
                  setSeasonDraft(event.target.value);
                  setCompetitionDetail({ season: event.target.value });
                }}
                onBlur={() => {
                  const normalized = normalizeSeason(seasonDraft);
                  setSeasonDraft(normalized);
                  setCompetitionDetail({ season: normalized });
                }}
              />
            </label>
            {isCup ? (
              ...
              <>
                <label>
                  ...
                  <input
                    ...
                    placeholder="1"
                    onChange={(event) => {
                      setRoundDraft(event.target.value);
                      setCompetitionDetail({
                        round: event.target.value ? `Journée ${event.target.value}` : "Journée 1",
                      });
                    }}
                    onBlur={() => {
                      const normalized = String(Math.max(1, Number(roundDraft) || 1));
                      setRoundDraft(normalized);
                      setCompetitionDetail({ round: `Journée ${normalized}` });
                    }}
                  />
                </label>
                <label>
        ...
        <EditorSection number="6" title="Statut et options">
          {form.status === "finished" && (
            <div className="match-editor-score-fields">
              <label>
                <span className="match-editor-label">Score USK</span>
                <input
                  type="number"
                  min="0"
                  step="1"
                  value={form.scoreFor ?? ""}
                  onChange={(event) =>
                    event.target.value && update({ scoreFor: Number(event.target.value) })
                  }
                  placeholder="0"
                />
              </label>
              <label>
                <span className="match-editor-label">Score adversaire</span>
                <input
                  type="number"
                  min="0"
                  step="1"
                  ...
                  onChange={(event) =>
                    event.target.value && update({ scoreAgainst: Number(event.target.value) })
                  }
                  placeholder="0"
                />
              </label>
