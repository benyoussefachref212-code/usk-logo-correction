...
> = {
  ...
  goalkeeper: {
    label: "Gardien",
    title: "Tenue Gardien",
    description: "La tenue officielle des gardiens de l’USK.",
    badge: "Gardien",
  },
  staff: {
    label: "Staff",
    title: "Tenue Staff",
    description: "La tenue officielle du staff et de la direction.",
    badge: "Staff",
  },
};
  ...
  async function upload(file?: File) {
    ...
    try {
      const image = await uploadAsset({
        data: {
          path: `kits/${slot}/kit-${Date.now()}-${file.name.replace(/[^a-zA-Z0-9.-]/g, "-")}`,
          file,
        },
      });
      await saveKit(slot, image);
      setError("");
    } catch (cause) {
    ...
    <div className="kit-preview-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div className={`kit-preview-dialog kit-preview-${slot}`} role="dialog" aria-modal="true" aria-labelledby="kit-preview-title">
        ...
        <div className="kit-preview-intro">
          ...
          <span className="kit-preview-index">0{(Object.keys(slotMeta) as KitSlot[]).indexOf(slot) + 1} / 05</span>
        </div>
