...
import { getTeamById, getVirtualKit, useTeams, type Team } from "../data/teams";
...
function AdminLiveMatchPage() {
  ...
  const homeTeam = match
    ? getTeamById(teams, match.homeTeamId) || getTeamById(teams, "usk")
    : undefined;
  const awayTeam = match ? getTeamById(teams, match.opponentTeamId) : undefined;
  const homeKitImage = homeTeam?.id === "usk" ? kits.find((kit) => kit.slot === "home")?.image : null;
      ...
      <main className="admin-effectif-content admin-dashboard-content">
        ...
        <section className="admin-live-score panel-block">
          ...
          <div className="admin-live-scoreboard">
            <div>
              <LiveTeamAsset team={homeTeam} kitImage={homeKitImage} fallback="USK" />
              <span>USK</span>
            ...
            <div>
              <LiveTeamAsset team={awayTeam} fallback={match.opponentShort} />
...
function LiveTeamAsset({
  ...
  const virtualKit = team ? getVirtualKit(team) : null;
  ...
  return (
    <div className="admin-live-team-asset" aria-label={`Logo et tenue de ${team?.name || fallback}`}>
      ...
      {kitImage ? (
        ...
        <div
          className={`virtual-shirt-preview admin-live-team-kit pattern-${virtualKit.pattern}`}
          style={
            {
              "--kit-primary": team?.colors || "#17356f",
              "--kit-accent": team?.accent || "#bd2437",
