...
async function loadRemoteMatches() {
  ...
  if (error) {
    console.error("[matches] Supabase load failed", {
      table: "public.usk_matches",
      error,
    });
    return;
  }
...
export async function updateMatchStatus(matchId: string, status: MatchStatus) {
  ...
  if (supabase) {
    const finishedScores =
      status === "finished"
        ? { score_for: current.scoreFor ?? 0, score_against: current.scoreAgainst ?? 0 }
        : {};
    const { error } = await supabase
      .from("usk_matches")
      .update({ status, ...finishedScores, updated_at: new Date().toISOString() })
      .eq("id", matchId);
    if (error) {
...
export async function deleteMatch(matchId: string) {
  ...
  if (supabase) {
    const { error } = await supabase.from("usk_matches").delete().eq("id", matchId);
    if (error) {
      throw Object.assign(new Error(error.message), {
        code: error.code,
        details: error.details,
  ...
  remoteMatches = remoteMatches.filter((match) => match.id !== matchId);
  if (typeof window !== "undefined") window.dispatchEvent(new Event(matchesUpdatedEvent));
}
...
export async function saveMatches(matches: Match[]) {
  if (supabase) {
    const rows = matches.map((match) => ({
      ...
      assistant_referee_1: match.assistantReferee1?.trim() || null,
      assistant_referee_2: match.assistantReferee2?.trim() || null,
      fourth_referee: match.fourthReferee?.trim() || null,
      updated_at: new Date().toISOString(),
    }));
    const { error } = await supabase.from("usk_matches").upsert(rows, { onConflict: "id" });
    if (error) {
      console.error("[matches] Supabase save failed", {
        table: "public.usk_matches",
